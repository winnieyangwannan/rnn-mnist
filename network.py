...
def get_perf(y_hat, y_loc, rule = None):
    ...
    if rule == 'mnist':
        y_loc = y_loc[:, 1:]
        y_loc_max = np.argmax(y_loc, axis=1)  # find the unit with biggest output
        y_hat_loc = y_hat[..., 1:]
        y_hat_loc_max = np.argmax(y_hat_loc, axis=1)  # find the unit with biggest output

        return y_loc_max == y_hat_loc_max

    else:
        y_hat_loc = popvec(y_hat[..., 1:])  # NN y
